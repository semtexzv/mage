...
/// 2. Collects built-in tools from `mage_sdk::tools::all()`
/// 3. Appends any discovered extension modules
/// 4. Calls `mage_sdk::app::run::run(modules)`
...
fn main() {{
    ...
    if let Some(cmd) = std::env::args().nth(1) {{
        match cmd.as_str() {{
            "snapshot" => {{
                ...
                mage_sdk::app::snapshot_cmd::run_snapshot(&args);
                return;
            }}
            "rebuild" => {{
                mage_sdk::app::rebuild::run_rebuild();
                return;
            }}
            _ => {{}}
        }}
    ...
    if !mage_sdk::upgrade::is_agent_mode() {{
        std::process::exit(match mage_sdk::app::monitor::run_monitor() {{
            std::process::ExitCode::SUCCESS => 0,
            _ => 1,
        }});
    }}

    mage_sdk::run_local(|| async {{
        let mut modules: Vec<Rc<dyn Module>> = mage_sdk::tools::all();
{mod_calls}
        mage_sdk::app::run::run(modules).await;
    }});
}}
"##
        ))
